module ALU_tb;
    ...
    initial begin
        ...
        // 8 bits
        a_8 = 8'd200; b_8 = 8'd100; #20;
        assert(result_8 == 8'd100) else $error("failed %d - %d, expected = 100, got = %d", a_8, b_8, result_8);

        // 16 bits
        a_16 = 16'd30000; b_16 = 16'd10000; #20;
        assert(result_16 == 16'd20000) else $error("failed %d - %d, expected = 20000, got = %d", a_16, b_16, result_16);

        // 32 bits
        a_32 = 32'd2432902008; b_32 = 32'd1048576; #20;
        assert(result_32 == 32'd2431853432) else $error("failed %d - %d, expected = 2431853432, got = %d", a_32, b_32, result_32);

        // Pruebas de Mod
        op = 4'b0100; // Mod

        // 2 bits
        a_2 = 2'd3; b_2 = 2'd2; #20;
        assert(result_2 == 2'd1) else $error("failed %d %% %d, expected = 1, got = %d", a_2, b_2, result_2);

        // 4 bits
        a_4 = 4'd9; b_4 = 4'd4; #20;
        assert(result_4 == 4'd1) else $error("failed %d %% %d, expected = 1, got = %d", a_4, b_4, result_4);

        // 8 bits
        a_8 = 8'd20; b_8 = 8'd6; #20;
        assert(result_8 == 8'd2) else $error("failed %d %% %d, expected = 2, got = %d", a_8, b_8, result_8);

        // 16 bits
        a_16 = 16'd100; b_16 = 16'd30; #20;
        assert(result_16 == 16'd10) else $error("failed %d %% %d, expected = 10, got = %d", a_16, b_16, result_16);

        // 32 bits
        a_32 = 32'd1000; b_32 = 32'd300; #20;
        assert(result_32 == 32'd100) else $error("failed %d %% %d, expected = 100, got = %d", a_32, b_32, result_32);

        // Pruebas de Div, Mult, And, Or, Xor, Sr, Sl solo con 4 bits
        op = 4'b0011; // div
        ...
        assert(result_4 == 4'd12) else $error("failed %d * %d, expected = 12, got = %d", a_4, b_4, result_4);
